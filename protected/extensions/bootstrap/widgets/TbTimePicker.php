...
class TbTimePicker extends CInputWidget
{
	...
	public function run()
	{
		...
		{
			if($this->form)
				$html =  $this->form->textField($this->model, $this->attribute, $this->htmlOptions);
			else
				$hmtl =  CHtml::activeTextField($this->model, $this->attribute, $this->htmlOptions);
		} else
			$html = CHtml::textField($name, $this->value, $this->htmlOptions);

		$html = CHtml::openTag('div',array('class'=>'input-append bootstrap-timepicker')).$html.'<span class="add-on"><i class="icon-time"></i></span></div>';
