<html>
	...
	<body>
		...
		<center>
			...
			<form action="batchbill2.php" method="post" enctype="multipart/form-data">
...
if( $_FILES['tsvfile']['name'] ) {
	...
	foreach( $rows as $row => $data ) {
		...
		$info[$row]['SRAP SESS LEN'] = filter_var( $row_data[18], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['SRAP GROUP SIZE'] = filter_var( $row_data[19], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['SRAP LANG CD'] = filter_var( $row_data[20], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['SCIN INVOICE MONTH'] = filter_var( $row_data[21], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['SCIN INVOICE DAYS'] = filter_var( $row_data[22], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['SCIN ATTEND CODE'] = filter_var( $row_data[23], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['SCIN ACT GRP SIZE'] = filter_var( $row_data[24], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['SCIN START TIME'] = filter_var( $row_data[25], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['SCIN END TIME'] = filter_var( $row_data[26], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['SCIN SCHOOL OTHER'] = filter_var( $row_data[27], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['/'] = filter_var( $row_data[28], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['SCIN VEND INVOICE'] = filter_var( $row_data[29], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['SCIN INVOICE AMT'] = filter_var( $row_data[30], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['SCIN SED PROG ID'] = filter_var( $row_data[31], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['/2'] = filter_var( $row_data[32], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['/3'] = filter_var( $row_data[33], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['/4'] = filter_var( $row_data[34], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );
		$info[$row]['/5'] = filter_var( $row_data[35], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH );



		// GET STUDENT ID FROM DB
		$sidq = "SELECT id FROM patients WHERE replace(student_id, '-', '') = '".$info[$row]['SRAP OSIS ID']."'";
		$getsid = mysql_query( $sidq );
		while($sid = mysql_fetch_assoc($getsid)){
			$studentid = $sid['id'];
		}
		...
		if( $studentid != '' ) {
			...
			$getpid = mysql_query($pqr);
			while($pid = mysql_fetch_assoc($getpid)){
				$providerid = $pid['id'];
			}
			...
			$getsess = mysql_query( $sessq );
			while($sess = mysql_fetch_assoc( $getsess ) ) {
				...
				if( $rowSet = mysql_fetch_assoc( $result ) ) {
				...
				$checknum = mysql_query( $cnq );
				$numsess = mysql_num_rows( $checknum );
				...
				//echo $gloc."<br>";
				$getloc = mysql_query($gloc);
				while($glc = mysql_fetch_assoc($getloc)){
